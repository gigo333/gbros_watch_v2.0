...
typedef struct {
	...
	int16_t _dc;
	int16_t _bl;
	spi_device_handle_t _SPIHandle; // @suppress("Type cannot be resolved")
} TFT_t;
...
bool spi_master_write_byte(spi_device_handle_t SPIHandle, const uint8_t* Data, size_t DataLength); // @suppress("Type cannot be resolved")
...
void delayMS(int ms);
void lcdInit(TFT_t * dev, int width, int height, int offsetx, int offsety, int rotation);
